...
dotenv.config();
import { campaignSchema,DBRowSchema } from '../schemas';


export const AllCampaignColumns = async(user_name:string,campaigns:string[]):Promise<campaignSchema[]> =>{
    return new Promise((resolve, reject) => {
        ...
        const campaigns_output:campaignSchema[] = [];
        ...
        connection.query(query, values, (error, results, fields) => {
            ...
            results.forEach((row: DBRowSchema) => {
...
export const DeleteCampaigns = async(user_name:string,campaigns:string[]):Promise<null> =>{
...
export const Campaigns = async (user_name:string):Promise<campaignSchema[]> =>{
    return new Promise((resolve, reject) => {
        const connection = mysql.createConnection({
            host: process.env.DATABASE_HOST,
            port: process.env.DATABASE_PORT ? parseInt(process.env.DATABASE_PORT, 10) : undefined,
        ...
        connection.query(query, values, (error, results, fields) => {
            ...
            results.forEach((row: { campaign: campaignSchema; }) => {
...
export const NumbersFromExcludedCampaigns = async (user_name:string, campaigns_to_exclude:string[]):Promise<string[] | any> => {
    return new Promise((resolve, reject) => {
        const connection = mysql.createConnection({
            host: process.env.DATABASE_HOST,
            port: process.env.DATABASE_PORT ? parseInt(process.env.DATABASE_PORT, 10) : undefined,
        ...
        connection.query(query, values, (error, results, fields) => {
            ...
            results.forEach((row: { target_number: string; }) => {
...
export const recordCampaignOnSentMessage = async (user_name:string,
